...
app.get(["/video", "/api/video"], async (req, res) => {
  ...
  if (await streamMediaBuffer(turboUrl, `AI Character Turbo ["${shortPrompt.slice(0, 25)}"]`, 'image/jpeg', 12000)) return;
  ...
  if (await streamMediaBuffer(defaultUrl, 'AI Character Standard', 'image/jpeg', 12000)) return;
