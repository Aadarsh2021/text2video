...
app.get(["/video", "/api/video"], async (req, res) => {
  ...
  async function streamRealVideoBuffer(urlStr, providerName, forceType = 'video/webm', timeoutMs = 12000) {
    try {
      const controller = new AbortController();
        ...
        if (results.length > 0) {
          ...
          if (infoRes.ok) {
            ...
            if (imageinfo?.url) {
              ...
              if (await streamRealVideoBuffer(imageinfo.url, `Wikimedia Real Video ["${q}"]`, mime, 15000)) return;
            }
          }
        }
      }
  ...
  if (await streamRealVideoBuffer(fallbackUrl, 'Verified HD Video Pool', 'video/mp4', 12000)) return;
