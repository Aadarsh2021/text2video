...
async function handleServerRequest(request, response) {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/video') {
      ...
      async function sendMedia(urlStr, providerName, forceType = null, timeoutMs = 5000) {
        try {
          const controller = new AbortController();
      ...
      if (await sendMedia(turboUrl, `AI Character Engine Turbo ["${shortPrompt.slice(0, 25)}"]`, 'image/jpeg', 5000)) return;
      ...
      if (await sendMedia(defaultUrl, 'AI Character Engine Standard', 'image/jpeg', 5000)) return;
      ...
      try {
        ...
        if (lexicaRes.ok) {
          ...
          if (data?.images?.length > 0) {
            ...
            if (hit?.src && await sendMedia(hit.src, 'Lexica Character AI', 'image/jpeg', 5000)) return;
          }
        }
      } catch (e) {}
      ...
      const picsumUrl = `https://picsum.photos/seed/${seed}/540/960`;
      if (await sendMedia(picsumUrl, 'Picsum Scene Engine', 'image/jpeg', 6000)) return;
