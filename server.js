...
async function handleServerRequest(request, response) {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/video') {
      ...
      async function sendRealMovingVideoBuffer(urlStr, providerName, forceType = 'video/webm', timeoutMs = 15000) {
        try {
          const controller = new AbortController();
          ...
          console.log(`[Real Moving Video Stream] ✅ ${providerName}: ${(arrayBuffer.byteLength / 1024).toFixed(0)}KB (${contentType})`);
          response.writeHead(200, {
          ...
          for (let i = 0; i < Math.min(results.length, 6); i++) {
            ...
            if (infoRes.ok) {
              ...
              if (imageinfo?.url && (mime.includes('webm') || mime.includes('mp4'))) {
                if (await sendRealMovingVideoBuffer(imageinfo.url, `Wikimedia Moving Video ["${videoSearchQuery}"]`, mime, 15000)) return;
              }
            }
          }
        }
      ...
      if (await sendRealMovingVideoBuffer(fallbackUrl, 'Verified HD Open Video Pool', 'video/mp4', 12000)) return;
